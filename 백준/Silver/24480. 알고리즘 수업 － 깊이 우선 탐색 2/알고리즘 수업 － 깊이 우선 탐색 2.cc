...
void DFS(vector<int>& visitCount, vector<vector<int>>& graph,
	const int& idx, int& order)
{
	visitCount[idx] = order++;

	for (const int& nextIdx : graph[idx])
	{
		if (!visitCount[nextIdx])
			DFS(visitCount, graph, nextIdx, order);
	}
}
