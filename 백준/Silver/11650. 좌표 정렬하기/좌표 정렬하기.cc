...
void Heapify(COORD* Data, const int& size, const int& parent)
{
	...
	if (left < size)
	{
		if (Data[largest].x < Data[left].x ||
			(Data[largest].x == Data[left].x && Data[largest].y < Data[left].y))
			largest = left;
	}

	if (right < size)
	{
		if (Data[largest].x < Data[right].x ||
			(Data[largest].x == Data[right].x && Data[largest].y < Data[right].y))
			largest = right;
	}
